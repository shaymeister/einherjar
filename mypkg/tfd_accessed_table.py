...
def accessedTable(
    ...
    data, sql, verbose = \
        data, sql, verbose
    if verbose >= 2:
        print("Extracting the accessed table")
    data[sql] = data[sql].astype(str).str.lower()

    # determine table data was selected from
    data['selected_from'] = data[sql].str.extract(r'(from\s\S*)')
    data['selected_from'] = \
        data['selected_from'].str.replace('from ', '', regex = False)
    ...
    if verbose >= 2:
        print("Extracted the accessed table\nMerging with original dataset")
